 ...
 */
package javasmartphone.p1u5.model;
...
public class Automobile implements Serializable {
	...
	public synchronized List<String> getAllOptionSetName() {
		List<String> res = new ArrayList<String>();
		for (int i = 0; i < optionSet.size(); ++i) {
			res.add(optionSet.get(i).getName());
		}
		return res;
	}
	
	public synchronized List<String> getOptionsAndPrices(String optionSetName) { 
		if (optionSetName == null) { return null; }
		for (int i = 0; i < optionSet.size(); ++i) {
			OptionSet aSet = optionSet.get(i);
			if (optionSetName.equals(aSet.getName())) {
				return aSet.getAllOptionListAndItsPrice();
			}
		}
		return null;
	}
